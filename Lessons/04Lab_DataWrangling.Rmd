...
ex3 <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phys.data.Tue <- filter(NTL.phys.data, 
                            lakename %in% c("Tuesday Lake"), 
                            year4 %in% c(1990:1999))

ex4 <- NTL.phys.data %in%
  filter(
    lakename == "Tuesday Lake", #comma will be like add, can also use &, |, 
    year4 %in% c(1990:1999)
  )
...
ex31_surface_mean <- NTL.phys.data %>%
  filter(
    lakename == "Tuesday Lake",
    year4 %in% 1990:1999,
    month(sampledate) == 7,
  ...
  summarize(mean_surface_temp = mean(temperature_C, na.rm = TRUE))
