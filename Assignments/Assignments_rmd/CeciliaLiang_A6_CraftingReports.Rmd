...
```{r project.setup, echo=TRUE, eval=TRUE, error=FALSE, results='hide', message=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r, echo=FALSE, eval=TRUE, warning=FALSE, fig.cap= "Monthly Ozone levels (2018)", fig.align='left', fig.height=3}
epa_data$Month <- factor(epa_data$Month, levels = 1:12, labels = month.abb)

OzoneLevel2018 <- epa_data %>%
  filter(Year==2018) %>%
...
OzoneLevel2018
```
```{r, echo=FALSE, eval=TRUE, warning=FALSE, fig.cap=" Monthly Ozone levels (2019)", fig.align='left', fig.height=3}
OzoneLevel2019 <- epa_data %>%
  filter(Year==2019) %>%
  ggplot(aes(x = Month, y = Ozone))+
  geom_boxplot()+
...
## Git Repository
https://github.com/jingxliang14/EDE_Fall2025  
